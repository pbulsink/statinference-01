...
Similarly, we can compare the expected variance of the means to the actual variance. The standard deviation of the population was set to equal 5, thus the variance will be `5^2 = 25`. The population variance can be estimated from the distribution of means using the formula `sd(sample_means)^2 * n`, where `n` is the number of samples. For our `n = 40`, this equals `r sd(sample_means)^2 * 40`, which is close to our expected 25. 
