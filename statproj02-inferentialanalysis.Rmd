...
#Introduction
This report will investigate the `R` dataset `ToothGrowth`. The data will be summarised and relationships between tooth length and supplement or dosage will be tested using confidence intervals and hypothesis tests, as described in the Coursera Data Science course on Statistical Inference.
...
We'll plot the data with boxplots, showing the relationship between dose and toothlength, broken up by supplement type. 
...
Thus, we can see that at no time does the confidence interval include 0, each increase in dosing increases tooth lengh, with statistically significant confidence.
...
Similarly, we find that we have a p value of `r mean(perm2>observed2)` for our 1 mg/day dosage (with an average tooth length difference of `r observed2`), and a p value of `r mean(perm3>observed3)` for 2 mg/day dosage (with an average difference of `r observed3`).
